                ...
                <li key={index}>
                    <label className={`flex items-center justify-between cursor-pointer transition-colors ${mobile ? 'py-3.5 px-2 border-b border-gray-100 active:bg-gray-50' : 'p-2 rounded-md hover:bg-gray-50'}`}>
                        ...
                        {counts && typeof counts[option] === 'number' && (
                            <span className="text-sm font-bold text-gray-600 bg-gray-100 px-2.5 py-1 rounded-full ml-3">
                                {counts[option]}
                            </span>
                        )}
                    </label>
    ...
    return (
        <div className={`relative ${className}`} ref={dropdownRef}>
            ...
            <button
                ...
                className={`flex items-center justify-between w-full h-[50px] bg-white border border-gray-300 rounded-lg px-4 text-left transition-all hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 ${selectedOptions.length > 0 ? 'text-gray-900' : 'text-gray-500'}`}
            >
            ...
            {isOpen && !isMobile && (
                <div className="absolute top-full mt-2 w-full bg-white border border-gray-200 rounded-xl shadow-xl z-50 p-2 min-w-[300px] animate-in fade-in zoom-in-95 duration-100">
                    <div className="flex justify-between px-2 pb-2 border-b border-gray-100 mb-2">
                        <button onClick={handleSelectAll} className="text-xs font-bold text-blue-600 hover:text-blue-700">Выбрать все</button>
                        <button onClick={handleDeselectAll} className="text-xs font-bold text-gray-500 hover:text-gray-700">Снять все</button>
                    </div>
                    {searchable && (
                        <div className="px-2 mb-2">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    placeholder="Поиск..."
                                    className="w-full bg-gray-50 text-sm pl-9 pr-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                />
                            </div>
                        </div>
                    )}
            ...
            {isOpen && isMobile && portalRoot && createPortal(
                <div className="fixed inset-0 z-[100] bg-white flex flex-col animate-in slide-in-from-bottom duration-300">
                    <header className="flex items-center justify-between p-4 border-b border-gray-200 sticky top-0 bg-white z-10">
                        ...
                        <button onClick={() => setIsOpen(false)} className="p-2 text-gray-500 rounded-full bg-gray-100 active:bg-gray-200">
                            <X className="h-6 w-6" />
                        </button>
                    </header>
                    
                    <div className="p-4 bg-gray-50 border-b border-gray-200 flex gap-3">
                        <button onClick={handleSelectAll} className="flex-1 py-2.5 bg-white border border-blue-200 text-blue-600 text-sm font-bold rounded-xl shadow-sm active:scale-95 transition-transform">Выбрать все</button>
                        <button onClick={handleDeselectAll} className="flex-1 py-2.5 bg-white border border-gray-200 text-gray-500 text-sm font-bold rounded-xl shadow-sm active:scale-95 transition-transform">Снять все</button>
                    </div>
                        ...
                        <div className="p-4 border-b border-gray-100">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-12 pr-4 py-3.5 bg-gray-100 border-none rounded-2xl text-base focus:ring-2 focus:ring-blue-500"
                                />
                            </div>
                        </div>
                    )}
                    ...
                    <footer className="p-4 border-t border-gray-200 bg-white shadow-[0_-4px_12px_rgba(0,0,0,0.05)] sticky bottom-0">
                        <button
                            ...
                            className="w-full py-4 bg-blue-600 text-white font-bold rounded-2xl shadow-lg active:scale-95 transition-all text-lg"
                        >
