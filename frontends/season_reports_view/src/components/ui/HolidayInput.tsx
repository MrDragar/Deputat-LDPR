    ...
    >
      {value.map((holiday, index) => (
        <span
          ...
          className={`inline-flex items-center gap-2 bg-slate-200 text-slate-800 text-sm font-medium pl-3 pr-1.5 py-1.5 rounded-md animate-in fade-in-0 zoom-in-95 duration-200 cursor-grab max-w-full break-words transition-all border-2 ${draggingIndex === index ? 'opacity-30' : 'opacity-100'} ${dragOverIndex === index && draggingIndex !== index ? 'border-blue-500' : 'border-transparent'}`}
        >
          ...
          <button
            ...
            className="rounded-full text-slate-500 hover:bg-slate-300 hover:text-slate-700 p-0.5 flex-shrink-0 cursor-pointer"
