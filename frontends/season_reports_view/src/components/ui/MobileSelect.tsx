  ...
  return (
    ...
    >
      <header className="flex items-center justify-between p-4 border-b border-gray-200 sticky top-0 bg-white z-10 shrink-0">
        ...
        <button
          onClick={onClose}
          className="p-2 text-gray-500 rounded-full hover:bg-gray-100 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 cursor-pointer"
          aria-label="Закрыть"
        >
          <X className="h-6 w-6" />
        </button>
          ...
          options.map(option => (
            <li key={option.value}>
              <button
                ...
                className="w-full flex items-center justify-between text-left px-4 py-3.5 rounded-lg text-gray-800 hover:bg-gray-100 cursor-pointer"
                role="option"
