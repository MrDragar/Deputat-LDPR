...
export const DetailedAnalytics: React.FC<DetailedAnalyticsProps> = ({ allData, regionName, onBack, selectedDeputyTypes, onDeputyTypeChange, selectedDates, onDatesChange }) => {
    ...
    const getDetailedStatsForDates = useCallback((dates: string[]) => {
        ...
        dates.forEach(date => {
            const periodData = allData.dataByDate[date] ? allData.dataByDate[date].statisticsByRegion[regionName] : undefined;
            if (!periodData) return;
    
            selectedDeputyTypes.forEach(type => {
                const typeData = periodData.data[type];
                if (typeData) {
            ...
            stats.totalCitizens.value += periodData.totalCitizens ?? 0;
        });
    
        return stats;
    }, [allData, regionName, selectedDeputyTypes]);
    ...
    const detailedComparisonStats = useMemo(() => {
     ...
     const historicalData = useMemo(() => {
        return allData.dateOptions.map(({ value: date }) => {
            const periodData = allData.dataByDate[date] ? allData.dataByDate[date].statisticsByRegion[regionName] : undefined;
            if (!periodData) {
                return { date, zs: 0, acs: 0, omsu: 0, totalDeputies: 0, interacting: 0, reception: 0, totalCitizens: 0 };
            }

            const filteredStats = { interacting: 0, reception: 0 };
            selectedDeputyTypes.forEach(type => {
                filteredStats.interacting += periodData.data[type]?.interacting ?? 0;
                filteredStats.reception += periodData.data[type]?.reception ?? 0;
            });

            const zsTotal = selectedDeputyTypes.includes('zs') ? (periodData.data.zs?.total ?? 0) : 0;
            const acsTotal = selectedDeputyTypes.includes('acs') ? (periodData.data.acs?.total ?? 0) : 0;
            const omsuTotal = selectedDeputyTypes.includes('omsu') ? (periodData.data.omsu?.total ?? 0) : 0;
            ...
            return {
                ...
                acs: acsTotal,
                omsu: omsuTotal,
                totalDeputies: zsTotal + acsTotal + omsuTotal,
                ...filteredStats,
                totalCitizens: periodData.totalCitizens,
            };
        });
    }, [allData, regionName, selectedDeputyTypes]);
    ...
    const historicalAppealsData = useMemo(() => {
        ...
        const chartData = dateOptionsToProcess.map(dateOption => {
            ...
            const regionData = allData.dataByDate[date] ? allData.dataByDate[date].statisticsByRegion[regionName] : undefined;
            
            const periodTotals: { [key: string]: string | number } = { date: dateOption.value };
            allTopics.forEach(topic => {
                periodTotals[topic] = regionData?.appeals[topic] || 0;
            });
            return periodTotals as ChartDataItem;
        });
        
       return { allTopics, colors, chartData };
    }, [allData, regionName, appealChartDates]);
    ...
    return (
        <div className="container mx-auto p-4 sm:p-6 md:p-8">
            <header className="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-brand-on-surface-secondary mt-1">Детальная аналитика по региональному отделению</p>
                    </div>
                </div>
            ...
            <div className="space-y-8">
                <ChartContainer title="Динамика показателей" subtitle="Исторические данные по количеству депутатов и граждан (на основе фильтра с главной страницы)">
                    <ResponsiveContainer>
                        <ComposedChart data={historicalData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                            ...
                            <Bar yAxisId="left" dataKey="totalDeputies" name="Всего депутатов" fill="#708090" />
                ...
                <AppealsStackedBarChart
                    ...
                    subtitle={`Анализ тем обращений в ${regionName} по периодам`}
